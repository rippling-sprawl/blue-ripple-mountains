...
export function ShowCard({
  ...
  const linkSlug = bandSlug ?? headliner?.slug;
  const href = linkSlug ? `/setlists/${linkSlug}/${show.slug}` : "#";
  const bandIsHeading = !bandSlug;
    ...
    >
      <div className="flex flex-col gap-1 p-4">
        <div className="flex items-start justify-between gap-2">
          <span className="text-xl font-bold leading-tight break-words">
            {bandIsHeading
              ? headliner?.name ?? "—"
